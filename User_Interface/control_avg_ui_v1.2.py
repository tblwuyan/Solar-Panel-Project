...
class DataStore:
    ...
    def update_arrow_state(self, light_values, avg):
        ...
        if all_below_minus30:
            self.arrow_state = [False, False, False, False]
            return
        light_to_drive = [[2,3],[1,2],[0,1],[0,3]]
...
class App:
    ...
    def save_csv_data(self):
        ...
        try:
            ...
            row = [current_time] + self.data.light + self.data.pressure + [self.data.roll, self.data.pitch, self.data.yaw]
    ...
    def draw_square(self):
        ...
        corners = [
            (cx - half, cy - half),  # 驱动器1 (F)
            (cx + half, cy - half),  # 驱动器2 (R)
            (cx + half, cy + half),  # 驱动器3 (B)
            (cx - half, cy + half)   # 驱动器4 (L)
        ]
        edges = [
            (cx, cy - half),  # 上 -> 光强2
            (cx + half, cy),  # 右 -> 光强1
        ...
        for i, (x, y) in enumerate(corners):
            ...
            canvas.create_text(x, y+22, text=['(F)','(R)','(B)','(L)'][i], font=('Arial', 8), fill='gray')
        ...
        for idx, (x, y) in enumerate(arrow_positions):
            ...
            canvas.create_line(x, y, tip_x, tip_y, 
                              ...
                              arrowshape=(head_size, head_size, head_size//2))
            if is_active:
                # 用简单圆环代替 stipple
                canvas.create_oval(x-6, y-6, x+6, y+6, outline='red', width=2)
                dir_names = ['上', '右', '下', '左']

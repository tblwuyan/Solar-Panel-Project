...
class PneumaticGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("气动控制系统 + 姿态显示 (实时)")
        ...
        # 存储最新数据
        self.pressures = [0.0, 0.0, 0.0, 0.0]   # F, R, B, L
        self.roll = 0.0
        self.pitch = 0.0
        self.yaw = 0.0
    ...
    def create_widgets(self):
        ...
        # ---- 角度显示 ----
        frame_angle = ttk.LabelFrame(self.root, text="JY901 姿态角", padding=10)
    ...
    def parse_line(self, line):
        # 只解析以 'P:' 或 'A:' 开头的行，忽略调试行
        if line.startswith("P:"):
            # 气压数据： P:F=101.2,R=100.5,B=102.0,L=99.8
            parts = line[2:].split(',')
            for part in parts:
                if '=' in part:
                    key, val = part.split('=')
            ...
            # 角度数据： A:roll=0.5,pitch=1.2,yaw=179.8
            try:
                data = line[2:].split(',')
                for item in data:
                    if '=' in item:
